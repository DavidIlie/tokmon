        ...
        <ChartShell height={height}>
          <ResponsiveContainer>
            <BarChart data={top} layout="vertical" margin={{ top: 4, right: 12, left: 4, bottom: 0 }}>
              ...
              <Tooltip content={modelTip} cursor={BAR_FILL} />
              <Bar dataKey="cost" radius={[0, 3, 3, 0]} isAnimationActive animationDuration={350}>
          ...
          <ChartShell height={height}>
            <ResponsiveContainer>
              <PieChart>
                ...
                <Tooltip content={provTip} />
              </PieChart>
            </ResponsiveContainer>
          </ChartShell>
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            <div className="tnum text-2xl text-fg-bright">{fmtCost(total)}</div>
        ...
        <div className="flex flex-col gap-4 pt-1">
          <div className="flex h-3 w-full overflow-hidden rounded-full bg-bg-3">
            {rows.map(r => r.value > 0 && (
              <div key={r.key} style={{ width: `${(r.value / total) * 100}%`, background: r.color }} title={`${r.label}: ${fmtTokens(r.value)}`} />
            ))}
          </div>
            ...
            {rows.map(r => (
              <div key={r.key} className="flex items-center justify-between gap-2 text-xs">
                ...
                <span className="text-fg">
                  ...
                  <span className="ml-1.5 text-fg-faint">{fmtPct(total > 0 ? r.value / total : 0)}</span>
                </span>
              </div>
            ))}
          </div>
