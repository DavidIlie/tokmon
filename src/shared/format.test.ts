...
import { formatCompactTokens, formatCurrency, formatResetAt, formatResetIn, resetParts } from './format'
...
test('formatResetIn derives from resetParts and preserves ceil rounding', () => {
  const now = Date.parse('2026-01-01T00:00:00.000Z')
  assert.equal(formatResetIn('2026-01-01T03:00:00.000Z', now), '3h 0m')
  assert.equal(formatResetIn('2026-01-01T00:00:30.000Z', now), '1m')
  assert.equal(formatResetIn('2026-01-02T03:04:00.000Z', now), '1d 3h')
  assert.equal(formatResetIn('2026-01-01T00:00:00.000Z', now), 'now')
})

test('menu-bar token counts stay compact across thousand, million and billion scales', () => {
  ...
  assert.equal(formatCompactTokens(1_000_000_000), '1B')
})
